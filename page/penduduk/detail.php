      ...
      <div class="content-wrapper">
        ...
        <section class="content">
          <?php 
				...
				echo '<tr>';
				echo '<th colspan="4" class="bg-navy" align-text="center">Detail data penduduk</td>';
				echo "</tr>";
				...
				echo '<tr>';
				echo '<td align="center"><a  href="edit.php?id='.$row["id"].'" class="btn btn-sm bg-orange pisah" 
				role="button">Edit Data</a>
				<a onClick=\'javascript: return confirm("Apakah anda yakin akan menghapus data ini?");\' 
				...
				echo '<tr>';				
				echo '<td align="center"><a type="button" href="printsurat.php?id='.$id.'" class="btn btn-primary btn-sm">Print Surat</a></td>';
				...
				echo '<tr>';
				echo '<form method="post" action="pindah.php?id='.$id.'">';
				echo '<td align="center"><select name="stts">
				...
				echo '<tr>';
				echo '<td align="center"><input type="submit" name="submit" value="Simpan" class="btn btn-sm bg-green"></td>';
				echo '<td>Nama Ibu</td>';
				echo '<td>:</td>';
				...
				//hubungan keluarga
				$querys = "SELECT * FROM penduduk WHERE noKK = '$nokk'";
				$results = mysqli_query($con,$querys);
				if (!$results){
				   die ("Could not query the database: <br />". mysqli_error($con));
				}
				echo '<table class="table table-striped table-bordered table-hover box" border="1">';
				echo '<tr>';
				echo '<th colspan="10" class="bg-navy">Data Kartu Keluarga</th>';
				echo '</tr>';
				echo "<tr>";
					echo '<th>No.</th>';
					echo "<th>No. KK</th>";
				...
				while ($rows = mysqli_fetch_array($results)){
				  echo '<tr>';
					  echo '<td>'.$i.'</td>'; 
					  echo '<td>'.$rows['noKK'].'</td>';
					  echo '<td>'.$rows['nik'].'</td>';
					  echo '<td>'.$rows['nama'].'</td>';
					  echo '<td>'.$rows['jenisKelamin'].'</td>';
					  echo '<td>'.$rows['tmptLahir'].'</td>';
					  echo '<td>'.$rows['tglLahir'].'</td>';
					  echo '<td>'.$rows['hubKel'].'</td>';
					  echo '<td><a href="detail.php?id='.$rows["id"].'" role="button">details</a></td>';
				  echo '</tr>';
				  $i = $i+1;
				  $samping=$samping+6;
				}
