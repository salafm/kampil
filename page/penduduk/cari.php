      ...
      <div class="content-wrapper">
		...
		<form method="GET" action="cari.php">
          <?php 
				...
				$query1 = "SELECT * FROM penduduk WHERE ((noKK like '%$cari%') OR (nik like '%$cari%') OR (nama like '%$cari%'))";
				$result1 = mysqli_query($con,$query1);
				$max = mysqli_num_rows($result1);
				
				// Assign the query
				$query = "SELECT * FROM penduduk WHERE ((noKK like '%$cari%') OR (nik like '%$cari%') OR (nama like '%$cari%')) LIMIT $startrow, 10"; 
				...
				echo '<input type="text" name="cari" size="16"  placeholder="Cari no. KK / NIK/ Nama" style="padding-bottom:13px" required autofocus/>';
